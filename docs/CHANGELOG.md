...
состоянием пакета: декларативный слой правил не активировался на реальной
установке, генерируемый nftables не загружался ядром, а RPC-плагин содержал
две возможности выполнения команд от root. Ниже — что исправлено.

Полный разбор: `docs/audit/2026-09-11-PRODUCTION-READINESS-AUDIT.md`.

### Исправлено — блокеры (функциональность не доезжала до роутера)

- **Упаковка `openstream-render.uc`** (`scripts/pack-ipk-a53.sh`, `package/openwrt/Makefile`).
  Рендерер не устанавливался пакетом, при этом init ожидал его в
  `/usr/libexec/`, а RPC вызывал по пути `/usr/share/openstream/`. Проверка
  существования всегда была ложной → init молча уходил в fallback, RPC вызывал
  несуществующий файл и возвращал успех.
- **Упаковка правил `.osrule.yaml`.** Каталог правил не устанавливался вообще,
  поэтому `--compile-rules` не запускался ни при каких условиях. Правила и
  каталог `/etc/openstream/rules` теперь включены в пакет.
- **Единый путь правил.** init читал `/usr/share/openstream/rules`, RPC писал в
  `/etc/openstream/rules`. Сведено к `/etc/openstream/rules`.
- **`redirect` вне NAT-цепочки** (`crates/openstream-backend-openwrt/src/nftables.rs`,
  `openstream-render.uc`). Statement `redirect` допустим только в цепочке
  `type nat`; правила генерировались в `type filter`, из-за чего `nft -f`
  отвергал файл целиком и НИ ОДНО правило не применялось. Redirect вынесен в
  отдельную цепочку `nat_prerouting`.
- **`reject` в prerouting.** `reject` допустим только в хуках
  input/forward/output. Блокировки QUIC/DoT перенесены в `filter_forward`.
- **TPROXY не был доведён до конца.** Метка ставилась, но не существовало ни
  `tproxy`-statement, ни `ip rule`/`ip route`. Теперь генерируется
  `tproxy ip to :10888` (квалификатор семейства обязателен при `ip daddr`), а
  init добавляет policy routing и снимает его при остановке.
- **Защита от петель (#74).** Проверялся `ct mark`, который нигде не
  устанавливался. Заменено на `meta mark` + `route.default_mark` в конфигурации
  sing-box; добавлен `iif lo return`.
- **TPROXY-inbound слушал loopback.** `listen: 127.0.0.1` для TPROXY
  неработоспособен (пакеты приходят с чужим destination-адресом) — заменено на
  `0.0.0.0`.
- **Конфликт имени таблицы.** Transparent-divert жил в `inet openstream`, которую
  движок правил пересоздаёт целиком, удаляя divert. Divert вынесен в отдельную
  таблицу `inet openstream_divert`.

### Исправлено — безопасность

- **Command injection от root в `import_subscription`** (`openstream.uc`).
  `user_agent`, `hwid` и URL подставлялись в строку shell без экранирования.
  Введены строгая валидация по allowlist и сборка аргументов массивом с
  экранированием одинарными кавычками.
- **Command injection от root в `test_server_latency`.** Поле `server` не
  валидировалось вообще. Добавлена проверка хоста и диапазона порта.
- **Отключена проверка TLS-сертификата** при загрузке подписок (`curl -k`) —
  флаг убран, добавлено ограничение `--proto =https`.
- **Невалидируемые CIDR/клиенты** в `render.uc` и `nftables.rs` подставлялись в
  nft-файл без проверки; некорректное значение ломало загрузку всей таблицы.
  Добавлена валидация IPv4/CIDR.
- **ACL сужен**: `"openstream": ["*"]` разделён на явные списки чтения и записи.

### Исправлено — ложная функциональность

- **`perform_update` была полностью фиктивной**: писала в лог «Загрузка SHA256…
  OK» и «успешно обновлён», не выполняя никаких действий. Теперь честно сообщает
  о нереализованности и предлагает обновление через opkg.
- **`check_updates`** возвращала захардкоженные версии. Теперь читает фактические
  версии из opkg и бинарников; удалённая проверка версий помечена как
  неподдерживаемая.
- **Cron указывал на несуществующий** `/usr/bin/openstream-autoupdate` → заменено
  на реальный `openstream-update-hostlists`.
- **`switch_singbox_variant`** сообщала о переключении сборки, не заменяя
  бинарник. Формулировка приведена в соответствие.
- **Монитор показывал случайные числа** (`rand()`) как «трафик». Выдуманные
  счётчики заменены на реальные данные о наполнении сетов nftables.

### Исправлено — надёжность

- **`CRON_MARK` не была определена** → `sed` с пустым шаблоном, дубликаты cron
  при каждом reload. Переменная задана, маркер согласован с RPC.
- **`$VPNSET` не определена** в `openstream-uci2yaml` → `geo_split_vpn_set` не
  попадал в конфиг. Исправлено на `$VPNSET_EU`.
- **Импорт несуществующего символа `dir`** из модуля `fs` (есть только `lsdir`)
  в `render.uc` и RPC — скрипты падали при исполнении.
- **Экранированный дефис в классах символов** регексов (`\-`) и диапазоны
  `\x00-\x1f` отвергаются рантаймом ucode, хотя `ucode -c` их пропускает.
- **Риск потери чужих заданий cron** при перезаписи `/etc/crontabs/root` —
  добавлена проверка успешности чтения перед записью.
- **Ошибки загрузки nft-файла глушились** (`2>/dev/null`). Теперь применяется
  `nft -c` (dry-run) с логированием причины в syslog.
- **`IpTable`**: пересортировка выполнялась на каждой вставке (O(n² log n)).
  Сортировка вынесена в `finalize()`.
- **`DomainTrie::find`**: аллокация `String` на каждую метку в hot-path.
  Добавлен быстрый путь без аллокации.

### Добавлено

- `scripts/check-nft-syntax.sh` — проверка генерируемых правил реальным парсером
  `nft -c`. Юнит-тесты проверяли лишь наличие подстрок, из-за чего ошибки
  `redirect`/`reject` оставались незамеченными.
- `scripts/check-ucode.sh` — компиляция И исполнение ucode-скриптов.
- Новые шаги CI: `nft-syntax` и `ucode-check`.
- `crates/openstream-backend-openwrt/examples/dump_nft.rs` — генератор набора
  правил для проверки синтаксиса.

### Изменено

- `DEPENDS` пакета: `ca-bundle` → `ca-bundle, dnsmasq-full, kmod-nft-queue,
  kmod-nft-tproxy, ip-full, ucode, ucode-mod-fs, ucode-mod-uci`. Ранее стоковый
  `dnsmasq` не умел `nftset`, а NFQUEUE/TPROXY-модули не объявлялись вовсе.

### Совместимость

- Требуется `dnsmasq-full` (ранее предполагался неявно).
- Каталог правил переехал в `/etc/openstream/rules` (единый путь).
- Таблица divert переименована в `inet openstream_divert`.
...
- **Исследование и реализация улучшений на основе анализа проблем Forkop (Issues #60, #63, #64, #72, #73, #84, #88, #91, #95, #110):**
  - **Защита от флаппинга DNS Failover (#64):** внедрен алгоритм гистерезиса с настраиваемым порогом сбоев (`failover_threshold`, default: 3) и интервалом восстановления (`failover_recovery_sec`, default: 30s) в ядре Rust, ucode и LuCI `services.js`.
  - **Прямой пропуск BitTorrent / P2P (#72):** добавлен селектор «BitTorrent Bypass» для портов 6881–6889 и 51413 TCP/UDP прямо в WAN (`bypass_p2p_ports`), предотвращающий перегрузку VPN-туннелей и блокировки хостеров.
  - **Изоляция и совместимость с гостевыми сетями (#84):** в цепочку `openstream_tproxy` фаервола добавлены явные правила разрешения TCP/UDP сокетов локальных прокси (8888, 1088), исключающие сбои TPROXY при политике `input=REJECT` в гостевых зонах fw4.
  - **Безопасная работа без IPv6 (IPv4-only Safe Fallback, #73, #91):** динамическая проверка доступности `/proc/sys/net/ipv6` и состояния `disable_ipv6`; автоматическое отключение IPv6 правил без сбоев маршрутизации.
  - **Исключения IP-подсетей (CIDR) и клиентов LAN (#88, #95):** поддержка динамических наборов `bypass_cidrs` и `bypass_clients` (`ip saddr @bypass_clients return`) в nftables и веб-интерфейсе.
  - **Кастомные заголовки подписок (#63, #50):** поддержка передачи пользовательского `User-Agent` (по умолчанию `ClashMeta/v1.18.0`) и аппаратного идентификатора `X-HWID` при загрузке подписок.
  - **mTLS аутентификация для DoH (#110):** поля для задания клиентского сертификата и приватного ключа в настройках защищенного DNS.
  - **Статические переопределения DNS (#60):** поддержка директив `dns_hosts` (`address=/<domain>/<ip>`) в рендерере dnsmasq.
